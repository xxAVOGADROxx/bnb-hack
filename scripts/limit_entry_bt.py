...
A limit-order entry idea, tested against our fee problem: instead of entering at
market when the signal fires (close[t]), park a limit at a dip below it —
close[t] * (1 - offset%) — and fill only if a later bar within a TTL touches it.
A fill
